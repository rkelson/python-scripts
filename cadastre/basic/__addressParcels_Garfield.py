...
def addressParcels_Garfield(newParcels, parcelFld):

    print(' ** Getting other addresses from Parcels that are not in the address points')

    exp = "{0} IS NULL AND {1} <> '' AND {1} IS NOT NULL".format('PARCEL_ADD', 'PROPERTY_ADDRESS')
    print ('Where Clause is', exp)

    arcpy.MakeFeatureLayer_management(newParcels, 'newParcelsFl', exp)
    print(arcpy.GetCount_management('newParcelsFl'),' records with Parcel addresses and no Address Point') # should be 2366 on 1/22/20
    ...
    def calcAddress():
        ...
        with arcpy.da.UpdateCursor('newParcelsFl', ['PARCEL_ADD', 'OrigAddress', 'PROPERTY_ADDRESS']) as rows:
