# Summit County address cleaner

import arcpy
from agrc import parse_address

def addressParcels_Summit(newParcels, parcelFld):

    print(' ** Getting other addresses from Parcels that are not in the address points')

    exp = "{0} IS NULL AND {1} <> '' AND {1} IS NOT NULL".format('PARCEL_ADD', 'Situs')
    ...
    def calcAddress():
        ...
        print(' ** Calculating PARCEL_ADD & OrigAddress for parcels')

        with arcpy.da.UpdateCursor('newParcelsFl', ['PARCEL_ADD', 'OrigAddress', 'Situs']) as rows:
    ...
    def cleanAddresses():
        ...
        with arcpy.da.UpdateCursor('newParcelsFl',['PARCEL_ADD', 'OrigAddress', 'OID@']) as rows:
            ...
            for row in rows:
                ...
                row[0] = row[0].strip().upper().replace("  "," ").replace(".","").replace(",","").replace(" DR DR"," DR").replace(" RD RD"," RD").replace(" ST ST"," ST").replace(" LOOP LOOP"," LOOP")
                rows.updateRow(row)
    cleanAddresses()
