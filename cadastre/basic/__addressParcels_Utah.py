...
def addressParcels_Utah(newParcels, parcelAddrFld):
    ...
    def calcAddress():
        ...
        with arcpy.da.UpdateCursor(newParcels, ['OrigAddress', parcelAddrFld]) as rows:
            ...
            for row in rows:
                ...
                if row[1] != None and row[1].find(',') > -1 and row[1][0].isdigit():

                    row[0] = row[1].split(',')[0].upper().strip().replace('  ',' ')     #OrigAddress
