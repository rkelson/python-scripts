...
def addressParcels_Wasatch(newParcels, parcelFld):

    print(' ** Getting other addresses from Parcels that are not in the address points')

    exp = "{0} IS NULL AND {1} <> '' AND {1} IS NOT NULL".format('PARCEL_ADD', 'PropertyAddress_1')
    ...
    def calcAddress():
        ...
        with arcpy.da.UpdateCursor('newParcelsFl', ['PARCEL_ADD', 'OrigAddress', 'PropertyAddress_1']) as rows:
